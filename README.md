...
## Current Sleek Deployment

The root `docker-compose.yml` and companion files also describe the currently
running Sleek deployment at `nbvpn.sleek.com`. Its OAuth2-Proxy, management,
relay, and TURN credentials are intentionally excluded from Git. Before using
that stack, follow [DEPLOYMENT-LOCAL-SECRETS.md](DEPLOYMENT-LOCAL-SECRETS.md)
to create the ignored runtime files from their templates.

The files under `config/` and the CrowdSec examples remain a separate hardened
reference configuration; they are not mounted by the live-aligned root Compose
file.
