...
```sh
python3 tests/run_change_management_url_tests.py
```
...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\tests\change_management_url.Tests.ps1
```
...
`.omx/plans/test-spec-change-netbird-management-url.md`; passing this local suite
