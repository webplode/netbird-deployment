...
function Install-NetBirdPackage {
    ...
    $packagePath = Join-Path $script:TemporaryDirectory $packageName

    Write-InstallerLog -Level INFO -Message "Downloading the official NetBird MSI for $($Architecture.PackageSuffix)."
    Invoke-DownloadWithRetry -Uri ([uri]$packageUrl) -Destination $packagePath

    if (-not (Test-NetBirdSignature -Path $packagePath)) {
    ...
    Copy-Item -LiteralPath $packagePath -Destination $CachedMsiPath -Force
...
function Invoke-NetBirdCommand {
    ...
    foreach ($line in $output) {
        if (-not [string]::IsNullOrWhiteSpace([string]$line)) {
            Add-Content -LiteralPath $InstallLog -Value ([string]$line) -Encoding UTF8
        }
    }
...
$ProfileName = 'SleekVPNTest'
$ManagementUrl = 'https://netbird-stg.sleek.com'
$ProgramFilesDirectory = $env:ProgramW6432
if ([string]::IsNullOrWhiteSpace($ProgramFilesDirectory)) {
    $ProgramFilesDirectory = $env:ProgramFiles
}
...
function Write-ProfileLog {
    ...
    Add-Content -LiteralPath $UserLog -Value $line -Encoding UTF8
}
